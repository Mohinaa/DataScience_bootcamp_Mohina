Unit 3 Week 1 Project Mohinur Abdullaeva 

1. What are the top 5 metros by population?

select
from (select metro_city, 
sum(population) as total_pop, rank() 
over (order by sum (population) desc )
as population_rank
...
select metro_city, floor(percent_students) 
as income_buckets, count (*)
from (select metro_city, ((population_age_$_9+population_age_10_14+population_age_15_17:: numberic/population;;numberic)*100 as percent _students
from public.census_metro_data_exp
where metro_city in ('New York', 'Los Angeles', 'Chicago', 'Houston', 'Dallas')
